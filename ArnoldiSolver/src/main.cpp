...
int main(int argc, char **argv){
    ...
    time_t t_ = time(NULL);
    logFile << "\n\n###" << put_time(localtime(&t_), "%d-%m-%Y_%H-%M-%S") << "\n\n";
    Streams::log.rdbuf(logFile.rdbuf());


    ProcessArgs args(argc, argv);

    Calculation::InitArnoldi(args.getInputFilePath());

    Calculation::setVerbose(true);
    Calculation::SetUpModel();
    Calculation::runASolver();
    Calculation::aPropertyExtractor();
...
//	const int NUM_EIGEN_VALUES = 200;// 1600;
//	const int NUM_LANCZOS_VECTORS = 400; // 3200;
